...
def generer_elements():
    ...
    q = fr"""Dans le frigo, il y a {a+b+c} yaourts. {a} sont à la banane, {b} sont à l'abricot et {c} sont à la vanille.\\
    Lisa en choisit un au hasard. Son frère Victor en choisit un au hasard à son tour.\\
    ...
    r =  fr"""
    \textbf {{a.}}  Lisa peut avoir choisi un yaourt à la banane, à l'abricot ou à la vanille. Une fois qu'elle a choisi, et comme il y a au moins 2 yaourts de chaque sorte, Victor a les mêmes 3 possibilités. Il y a donc $3\times3=9$ issues possibles.\\
    ...
    {fraction2epreuvessame(c,tot)}.\\
    La probabilité qu'ils choisissent le même parfum est la somme des probabilités des issues (B,B), (A,A) et (V,V), soit :\\
    $\dfrac{{{a*(a-1)}}}{{{tot*(tot-1)}}}+\dfrac{{{a*(a-1)}}}{{{tot*(tot-1)}}}+\dfrac{{{a*(a-1)}}}{{{tot*(tot-1)}}}=\dfrac{a*(a-1) + b*(b-1) + c*(c-1)}{tot*(tot-1)}={fraction_to_latex(Fraction(a*(a-1) + b*(b-1) + c*(c-1), tot*(tot-1)))} $.\\
    ...
    while a+b+c+d not in primelist:
        a,b,c,d = [randint(1,6) for i in range(4)]
    while e+f+g+h not in primelist or e+f+g+h == a+b+c+d:
        e,f,g,h = [randint(1,6) for i in range(4)]
    ...
    Il y a {a} paires de chaussettes rouges, {b} paires de chaussettes vertes, {c} paires de chaussettes bleues et {d} paires de chaussettes noires \\
    Dans le deuxième tiroir, Rémi a mis des T-shirt. Il y a {e} T-shirt rouges, {f} T-shirt verts, {g} T-shirt bleus, {h} T-shirt noirs\\
    ...
    \textbf {{a.}} Quelle est la probabilité que Rémi ait choisi un T-shirt rouge ?\\
    \textbf {{b.}} Quelle est la probabilité que Rémi ait choisi des chaussettes et un T-shirt bleus ?\\
    ...
    r = fr"""
    \textbf {{a.}} Il y a {e} T-shirt rouges, sur {t2} T-shirts possibles. Comme Rémi fait son choix au hasard, les {t2} t-shirts sont équiprobables. La probabilité de choisir un t-shirt rouge est donc ${fraction_to_latex(Fraction(e,t2))}$
    \textbf {{b.}}  Il y a {c} paires de chaussettes bleues et il y a {t1} paires de chaussettes possibles.
