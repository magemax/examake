...
def generer_elements():
    ...
    Lisa en choisit un au hasard. Son frère Victor en choisit un au hasard à son tour, et on s'intéresse aux yaourts obtenus par les deux frères et soeurs.\\
    ...
    \textbf {{b.}}  Comme le nombre de yaourts est différent d'un parfum à l'autre, Lisa n'a pas la même probabilité de choisir n'importe quel parfum. On en déduit qu'il est impossible que les issues (B,B) et (A,A) aient la même probabilité.\\
    ...
    r = fr"""
    \textbf {{a.}} Il y a {g} T-shirt noirs, sur {t2} T-shirts possibles. Comme Rémi fait son choix au hasard, les {t2} t-shirts sont équiprobables. La probabilité de choisir un t-shirt noir est donc ${fraction_to_latex(Fraction(g,t2))}$
    \textbf {{b.}}  Il y a {a} paires de chaussettes rouges et il y a {t1} paires de chaussettes possibles.
    La probabilité de choisir une paire de chaussettes rouges est : ${fraction_to_latex(Fraction(a, t1))}$.\\
    Il y a {e} T-shirt rouges sur {t2} T-shirt possibles. La probabilité de choisir un des T-shirt rouges est : 
    ${fraction_to_latex(Fraction(e,t2))}$.\\
