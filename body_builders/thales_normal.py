...
def ratios():
    #Renvoie quatre nombres a,b,c,d, tels que a/b=c/d et tous avec au plus une décimale
    #ad=bc
    #On met du ad = 2*5 * autres nombres premiers  /100 
    # a,c prend le 2/10, b,d prend le 5/10
    autres_premiers =[3,7,11,13,17,23]
    # Ensuite, euh je sais pas moi, il me faut 3 nombres premiers ? 
...
def generer_elements(typ=0): #typ=0 pour direct, sinon 1
    ...
    cotes=[distbase,(distbase*ratio).normalize(), (distbase*ratio2).normalize(), (distbase*ratio*ratio2).normalize()]
    ...
    corrtable+=[f"Les droites ({dnames['AB']}) et ({dnames['NM']}) sont parallèles."]
